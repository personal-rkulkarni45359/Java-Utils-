...
public class JImage {
	...
	public void writeJImageAsIP01(String outputPath) throws IOException {
	...
	public void combineHorizontal(JImage image) throws IOException
	{
		Combine2Transform combiner = BaseTransform.getCombiner(this,image);
		JImage temp = combiner.combineX();
		this.bufferedImage= temp.bufferedImage;
	}

	public void combineVertical(JImage image) throws IOException
	{
		Combine2Transform combiner = BaseTransform.getCombiner(this,image);
		JImage temp = combiner.combineY();
		this.bufferedImage= temp.bufferedImage;
		
		
	}
	public void combineAuto(JImage image) throws IOException
	{
		Combine2Transform combiner = BaseTransform.getCombiner(this,image);
		JImage temp = combiner.autoCombine();
		this.bufferedImage= temp.bufferedImage;
	}
	...
	public void writeEncryptedIP01(String filePath) throws IOException {
		String finalPath = filePath;
		if(!filePath.contains(JConstants.IP01))
		{
			 finalPath = filePath + JConstants.IP01_EXTENSION;
		}
		byte[] encyptedBytes = encryptImage();
		JImageTranslator.writeEncryptedBytes(encyptedBytes, finalPath);
	}

	public void writeEncryptedIJ01(String filePath) throws IOException {
		
		String finalPath = filePath;
		if(!filePath.contains(JConstants.IJ01_EXTENSION))
		{
			 finalPath = filePath + JConstants.IJ01_EXTENSION;
		}
		byte[] encyptedBytes = encryptImage();
		JImageTranslator.writeEncryptedBytes(encyptedBytes, finalPath);
	}
